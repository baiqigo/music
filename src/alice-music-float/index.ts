  ...
  <div class="bar bar-bottom" id="ml-bar-bottom">
    ...
    <div id="ml-btn-heart" style="cursor:pointer;display:flex;align-items:center;justify-content:center;">
      ...
      function playMoonTransition(onComplete: () => void) {
        ...
        moonEl.style.left = startX + 'px';
        moonEl.style.top = startY + 'px';
        ...
        for (let i = 0; i < starCount; i++) {
          ...
          const size = 16 + Math.random() * 24; // 更大更明显
          ...
          starInfos.push({
            el: star,
            progress: 0.15 + Math.random() * 0.75, // 最终停在轨迹 15%~90% 位置
            delay: 0.05 + Math.random() * 0.3, // 延迟产生拖尾
            offX: (Math.random() - 0.5) * 90, // 横向散布
            offY: (Math.random() - 0.5) * 90, // 纵向散布
          });
        }
        ...
        function tick() {
          ...
          moonEl.style.left = mx + 'px';
          moonEl.style.top = my + 'px';
          // 月亮逐渐出现
          moonEl.style.opacity = rawT < 0.08 ? String(rawT / 0.08) : '1';

          // 白色遮罩：对角线渐变分界线跟随月亮对角线进度
          // 将月亮的 2D 位置投影到 135° 对角线上的百分比
          const diag = Math.sqrt(panelW * panelW + panelH * panelH);
          const projectedDist = (mx + my) / Math.sqrt(2); // 投影到 135° 方向
          ...
          for (const si of starInfos) {
            ...
            si.el.style.left = sx + 'px';
            si.el.style.top = sy + 'px';
