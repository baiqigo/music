  ...
  <div id="float-panel" class="float-panel hidden">
    ...
    <div id="main-page" class="panel-body">
      ...
      <div class="controls">
        ...
        <button id="btn-fav" class="icon-btn ctrl-side-btn" title="收藏"><i class="fa-regular fa-heart"></i></button>
      </div>
    </div>
...
/* 月光白：overlay-page 在 ml-card 之上显示 */
...
.ml-card .ml-one .ml-header .ml-header-icon {
  cursor: pointer;
  transition: fill 0.15s;
}
.ml-card .ml-one .ml-header .ml-header-icon:hover {
  fill: rgba(29, 28, 28, 0.95);
}
.ml-card .ml-one .ml-title {
  width: 70px;
...
.ml-card .ml-one .ml-music .ml-note {
  fill: rgba(82, 79, 79, 0.5);
}
.ml-card .ml-one .ml-music .ml-cover-img {
  position: absolute;
  top: 0; left: 0;
...
.ml-card .ml-one .ml-bar.ml-bar-bottom {
  margin: auto auto 8px;
  width: 100%;
  padding: 2px 23px;
}
.ml-card .ml-one .ml-bar .ml-color {
  fill: rgba(82, 79, 79, 0.829);
  cursor: pointer;
  transition: fill 0.15s;
}
.ml-card .ml-one .ml-bar .ml-color:hover {
  fill: rgba(29, 28, 28, 0.95);
}
.ml-card .ml-one .ml-bar .ml-color1 {
  fill: rgba(29, 28, 28, 0.829);
  cursor: pointer;
  transition: fill 0.15s;
}
.ml-card .ml-one .ml-bar .ml-color1:hover {
  fill: rgba(29, 28, 28, 0.95);
}
/* 上一首按钮翻转（参考 CSS） */
.ml-card .ml-one .ml-bar .bi:first-child {
  transform: rotate(180deg);
}
.ml-card .ml-one .ml-bar.ml-bar-bottom .ml-color1:first-child {
  transform: rotate(0deg);
}

/* ---- 樱花粉主题占位 ---- */
.float-panel.theme-sakura {
  /* 樱花粉 —— 占位，后续美化 */
}
...
$(() => {
  ...
  const $iframe = createScriptIdIframe()
    ...
    .on('load', () => {
      ...
      const THEMES = ['默认', '月光白', '樱花粉'] as const;
...
<div class="ml-one">
  <div class="ml-header">
    <svg id="ml-btn-settings" viewBox="0 0 16 16" class="ml-color1 ml-header-icon bi bi-gear" fill="currentColor" height="16" width="16" xmlns="http://www.w3.org/2000/svg">
  ...
  <span class="ml-title">Music</span>
  <div class="ml-music">
    <svg viewBox="0 0 16 16" class="ml-note bi bi-music-note" fill="currentColor" height="18" width="18" xmlns="http://www.w3.org/2000/svg">
    ...
    <img class="ml-cover-img hidden" alt="" />
  </div>
  <span class="ml-name" id="ml-song-title">暂无歌曲</span>
  <span class="ml-name1" id="ml-song-artist">--</span>
  <div class="ml-bar" id="ml-bar-controls">
    <svg id="ml-btn-prev" viewBox="0 0 16 16" class="ml-color bi bi-fast-forward-fill" fill="currentColor" height="16" width="16" xmlns="http://www.w3.org/2000/svg">
    ...
    <svg id="ml-btn-play" viewBox="0 0 16 16" class="ml-color bi bi-caret-right-fill" fill="currentColor" height="18" width="18" xmlns="http://www.w3.org/2000/svg">
      <path d="m12.14 8.753-5.482 4.796c-.646.566-1.658.106-1.658-.753V3.204a1 1 0 0 1 1.659-.753l5.48 4.796a1 1 0 0 1 0 1.506z"></path>
    </svg>
    <svg id="ml-btn-next" viewBox="0 0 16 16" class="ml-color bi bi-fast-forward-fill" fill="currentColor" height="16" width="16" xmlns="http://www.w3.org/2000/svg">
      <path d="M7.596 7.304a.802.802 0 0 1 0 1.392l-6.363 3.692C.713 12.69 0 12.345 0 11.692V4.308c0-.653.713-.998 1.233-.696l6.363 3.692Z"></path>
      <path d="M15.596 7.304a.802.802 0 0 1 0 1.392l-6.363 3.692C8.713 12.69 8 12.345 8 11.692V4.308c0-.653.713-.998 1.233-.696l6.363 3.692Z"></path>
    </svg>
  </div>
  <div class="ml-bar ml-bar-bottom" id="ml-bar-bottom">
    <svg id="ml-btn-shuffle" viewBox="0 0 16 16" class="ml-color1 bi bi-shuffle" fill="currentColor" height="14" width="14" xmlns="http://www.w3.org/2000/svg">
    ...
    <svg id="ml-btn-playlist" viewBox="0 0 16 16" class="ml-color1 bi bi-music-note-list" fill="currentColor" height="14" width="14" xmlns="http://www.w3.org/2000/svg">
    ...
    <svg id="ml-btn-heart" viewBox="0 0 16 16" class="ml-color1 bi bi-suit-heart" fill="currentColor" height="14" width="14" xmlns="http://www.w3.org/2000/svg">
      ...
      function enterMoonlightLayout() {
        ...
        const mlBtnPlay = mlCard.querySelector('#ml-btn-play') as SVGElement;
        const mlBtnNext = mlCard.querySelector('#ml-btn-next') as SVGElement;
        const mlBtnShuffle = mlCard.querySelector('#ml-btn-shuffle') as SVGElement;
        ...
        mlBtnPlay?.addEventListener('click', () => iframeDoc.getElementById('btn-play')!.click());
        mlBtnNext?.addEventListener('click', () => iframeDoc.getElementById('btn-next')!.click());
        mlBtnShuffle?.addEventListener('click', () => iframeDoc.getElementById('btn-repeat')!.click());
      ...
      function syncMoonlightInfo() {
        ...
        const mlCover = mlCard.querySelector('.ml-cover-img') as HTMLImageElement;
        const mlNote = mlCard.querySelector('.ml-note') as SVGElement;
      ...
      function applyTheme(name: ThemeName) {
        ...
        floatPanel.classList.remove('theme-moonlight', 'theme-sakura');
        if (name === '月光白') {
          ...
          if (name === '樱花粉') floatPanel.classList.add('theme-sakura');
          currentMoonlight = false;
        }
        themeToggle.textContent = name;
      }

      // 初始化：从 localStorage 读取上次主题
      try {
