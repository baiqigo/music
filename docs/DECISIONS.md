...
7. 网易云搜索结果跳过 `checkAudioPlayable`：Meting URL 为可信代理地址（302→CDN），`checkAudioPlayable`
   的 3-6 秒超时在并行校验时会误过滤有效结果。QQ 音乐仍需校验（落月 V3 URL 可能为空路径或不可播放）。
8. 网易云 Meting API 双实例 fallback：主 `meting-api-omega.vercel.app`（公共），备 `qq-music-beige.vercel.app`
   （用户自部署 xizeyoupan/Meting-API fork）。主实例失败自动切备。
9. 不采用 Cookie 方案：公开发布的脚本不适合绑定个人网易云 VIP 账号，避免账号风控风险和维护负担。VIP 歌曲在免费架构下只能播放 30 秒试听，免费歌曲为完整版。
